...
def generate_full_report():
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write("分析対象データ: 2015年〜2025年（全629問・Geminiによる実務的分類済）\n")
            f.write("解析モデル: **AI融合型ハイブリッドエンジン (scikit-learn RandomForest + 統計分析)**\n\n")
            
            f.write("## 1. 総合分析サマリー\n")
            f.write("今回の分析では、従来の統計的な「出題周期」に加え、**scikit-learnを用いた機械学習モデル**を導入しました。過去11年分のデータから「どのような特徴を持つ論点が出題されやすいか」をAIが学習し、2026年の予測精度を大幅に向上（バックテスト比 +5%）させています。さらに、2026年の法改正（行政書士法改正、デジタル社会対応）による「介入効果」を数値化して算出しました。\n\n")
